...
支持多种文档格式的加载和预处理
    ...
    1. 按文件类型读取成 LangChain 的 Document 列表（txt/csv/pdf/docx/xlsx）
    2. 给每个 Document 补充 metadata（source、file_type、file_name、page_number 等）
    3. 支持单文件、目录批量加载
    4. 支持 load_academic_csv（按列拼装文本和元数据）
    - 不负责向量化、不负责写 Milvus
"""

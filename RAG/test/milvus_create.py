...
class MilvusManager:
    ...
    def __init__(self, host: str, port: str, collection_name: str, embedding_model: str, dashscope_api_key: str):
        ...
        self.alias = MILVUS_ALIAS
        self.connect_milvus()  # 建立Milvus连接
        self.create_collection_if_not_exists()  # 创建集合（如果集合不存在）

    def connect_milvus(self):
        """建立与 Milvus 服务器的连接。"""
        connections.connect(alias=self.alias, host=self.host, port=self.port)  # 连接到 Milvus 数据库
        print(f"已连接到 Milvus，地址为 {self.host}:{self.port}, alias={self.alias}")  # 输出连接成功消息
    ...
    def create_collection_if_not_exists(self):
        ...
        if not utility.has_collection(self.collection_name, using=self.alias):
            ...
            collection = Collection(name=self.collection_name, schema=schema, using=self.alias)  # 创建集合
            index_params = {
        ...
        self.collection = Collection(name=self.collection_name, using=self.alias)  # 设置集合属性
